...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Jianlong Huang" };
...
const int NUM_PARTICLES = 1024 * 1024 * 2;
...
InitLists( )
{
	...
	// ****************************************
		...
		glColor3f( .18f, .89f, 0. );	// 0. <= r,g,b <= 1.
		glPushMatrix( );
			glTranslatef( 300.f, -800.f, 1.f );
	...
	// do this for yourself...
		glColor3f(.83f, .13f, 1.);	// 0. <= r,g,b <= 1.
		glPushMatrix();
			glTranslatef(-400.f, -800.f, 0.f);
			glutWireSphere( 300.f, 100, 100);
		glPopMatrix();
